...
class NIDAQ_Acquisition:
    ...
    def __init__(self):
        ...
        self.task_trig = None # TODO ajouté ici
    ...
    def send_signals_to_daq_single_channel(self, tensions_library,
        ...
        self.timepoints = timepoints # Useless naw
        ...
        self.channel_co.co_pulse_term = self.daq_channels["co_terminal"]
            ...
            # Positions are made one by one naw
        self.task_co.timing.cfg_implicit_timing(samps_per_chan = 1)
    ...
    def arm_task(self):
        ...
        if self.state != "ready":
            raise RuntimeError(f"Cannot start acquisition from state '{self.state}'. Must be 'ready'.")
    ...
    def trigger_acquisition(self, timeout = 1.0):
        """Start the counter output (CO) task used to trigger all other tasks.

        The CO output is intentionally kept instead of switching to a digital
        output (DO) to preserve the existing hardware routing (PFI) and minimize
        changes to the microscope control software.
        """
        if not (self.state == "armed" or self.state == "running") :
            raise RuntimeError(f"Cannot start acquisition from state '{self.state}'. Must be 'armed' or 'running.")
            
        self.task_co.start()
        self.task_co.wait_until_done(timeout=timeout)
        self.task_co.stop()
